...
def register():
    ...
    if request.method == 'POST':
        ...
        if account:
            ...
            cursor.execute('INSERT INTO register1 (username, email, password, otp) VALUES (%s, %s, %s, %s)', (username, email, arr[0],otp))
            mysql.connection.commit()
...
def check_email():
    try:
        ...
        email_forget_pass(account[1],account[3])   
        if account:
            session['bulk'] = account  
            session['id'] = account[0]
        ...
        return render_template('login.html',msg ="Email account not registered!")
...
def check_otp():
    ...
    account = session['bulk']

    if account[3] != None and int(otp) == int(account[3]):
        cursor = mysql.connection.cursor()
        cursor.execute('UPDATE `register1` SET `otp` = % s', (str(random.randint(10000,99999)),))
        mysql.connection.commit()
        
        
        return  redirect('/home')
    else:
        return render_template('login.html', msg = "Otp did not match!")
...
def year():
      ...
      arr = []
      arr1=["January","February","March","April","May","June","July",
            "August","September","October","November","December"]
     
      texpense =list(texpense)
      ...
      return render_template("year.html", texpense =arr, expense = expense,  total = total ,
                           t_food = t_food,t_entertainment =  t_entertainment,
                           t_business = t_business,  t_rent =  t_rent, 
                           t_EMI =  t_EMI,  t_other =  t_other )
...
def email_forget_pass(email,otp):
...
def send_email_if_exceeded(arr,err_expense, email):
    ...
    for i in arr:
        ...
        cursor.execute('SELECT * FROM link WHERE  category = %s', (i,))
    ...
    for i in array:
        ...
        str2 += f"Exceeded expenditure:{i[0][0]}"+"\n"+f"checkout the link to control your expenditure {i[0][1]}"+"\n"
        k +=1
